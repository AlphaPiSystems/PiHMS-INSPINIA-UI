    ...
    this.http.get<any>('assets/data/db.json').subscribe({
      next: (data) => {
        if (data && data.district) {
          this.allDistricts = data.district;
          this.tableService.setItems(this.allDistricts, this.tableService.pageSize);
        }
      },
      error: (err) => {
        console.error('Error loading districts from JSON:', err);
      }
    });
  }
}
